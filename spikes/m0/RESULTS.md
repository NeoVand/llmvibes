...
consistently ~2× *slower* per step than syncing every step (e.g. 1040 vs 462
...
architecture, and it's the *fast* path, not a compromise.
...
1. **GPT-convention zero-init of output projections (wo/mlpFc2)** makes those
   grads *mathematically* zero at step 0 (dPre = dOut @ 0ᵀ).
2. A **frozen** zero-init base blocks the attention block *forever* — so LoRA
...
Everything is *correct*; only throughput misses the aspirational gate (~4.4k
