...
function ensureInit(): Promise<void> {
  ready ??= (async () => {
    ...
    log(`jax-js devices: ${devices.join(', ')} (default: ${devices.includes('webgpu') ? 'webgpu' : devices[0]})`);
    try {
...
function initParams(cfg: Cfg, key: any, randomOut = false) {
  ...
  const outInit = (shape: number[]) =>
    randomOut ? random.uniform(nk(), shape, { minval: -0.2 * s, maxval: 0.2 * s }) : (nk().dispose(), np.zeros(shape));
  const params: any = {
...
async function trainBench(label: string, cfg: Cfg, useJit: boolean, flat: boolean, timedSteps = 10) {
  await ensureInit();
  ...
  const jitStep = useJit
    ? jit((p: any, a: any, b: any, c: any) => valueAndGrad((pp: any) => lossFn(pp, cfg, a, b, c, flat))(p))
    : null;
  ...
  const runStep = (stepIdx: number, sync: boolean): number => {
    ...
    const [lossVal, grads] = jitStep
      ...
      : valueAndGrad((pp: any) => lossFn(pp, cfg, tokenOH.ref, posOH.ref, targetOH.ref, flat))(tree.ref(params));
...
function lossFnLora(ad: any, base: any, cfg: Cfg, tokenOH: any, posOH: any, targetOH: any, scale: number) {
  const headDim = cfg.nEmbd / cfg.nHead;
  const seqLen = cfg.blockSize;
...
async function loraBench(label: string, cfg: Cfg, r: number, timedSteps = 20) {
  ...
  // (B=0 makes |grad aq|₁ = 0 at init — that is CORRECT, not a bug).
  {
    ...
    const [lv, gr] = jitStep(tree.ref(adapters), tree.ref(base), tokenOH.ref, posOH.ref, targetOH.ref);
    lv.dispose();
  ...
  const runStep = (stepIdx: number, sync: boolean): number => {
    const { tokenOH, posOH, targetOH } = makeBatch(cfg, stepIdx);
    const [lossVal, grads] = jitStep(tree.ref(adapters), tree.ref(base), tokenOH.ref, posOH.ref, targetOH.ref);
  ...
  return new Promise<void>((resolve) => {
    ...
    w.onmessage = (e: MessageEvent) => {
      ...
      if (d.type === 'log') log('[worker] ' + d.msg);
        ...
        result('worker', { ...d.payload, wallMs, mainThreadTicks: ticks, expectedTicks: Math.round(wallMs / 50) });
        done();
      } else {
