		...
		const model = new FakeChatModel([
			new AIMessage({
				...
				tool_calls: [
					{
						name: 'search_course',
						args: { query: 'train a BPE tokenizer' },
						id: 'call_1',
						type: 'tool_call'
					}
				]
			}),
			new AIMessage('You can watch BPE merges build a vocabulary live [[section-2-2]].')
		]);
