<script lang="ts">
	...
	import { Play, Loader2, ThumbsUp, Scale } from 'lucide-svelte';
...
>
	...
	{:else}
		...
		{#if pair}
			<div class="grid gap-3 sm:grid-cols-2" class:opacity-60={sampling}>
				{#each pair as candidate, i (i)}
					<button
						class="vote-card rounded-xl border p-3 text-left"
						class:pulse={justChose === i}
						style="border-color: {justChose === i
							? 'var(--color-tip)'
							: 'var(--color-border-light)'}; background: var(--color-surface);"
						disabled={sampling}
					...
					>
						...
						<div class="text-[13px] leading-relaxed" style="color: var(--color-text);">
							{candidate.text}
						</div>
					</button>
				{/each}
			...
			<div class="mt-2 flex items-center justify-center">
				<button class="lab-btn" disabled={sampling} onclick={() => vote('tie')}>
		...
		{#if stats}
			<div class="mt-4 rounded-lg border p-3" style="border-color: var(--color-border-light);">
				<div
					class="mb-2 text-[11px] font-bold tracking-wide uppercase"
					style="color: var(--color-text-muted); letter-spacing: 0.08em;"
				>
				...
				{#each [{ label: 'avg length (words)', c: stats.chosenLen, r: stats.rejectedLen, fmt: (v: number) => v.toFixed(1) }, { label: 'distinct-word ratio', c: stats.chosenDistinct, r: stats.rejectedDistinct, fmt: (v: number) => v.toFixed(2) }] as row (row.label)}
					<div class="mb-2 last:mb-0">
						<div class="mb-1 text-[11px]" style="color: var(--color-text-secondary);">
							{row.label}
						</div>
						<div class="grid grid-cols-[3.5rem_1fr_2.5rem] items-center gap-2">
							<span class="text-[10px] font-semibold" style="color: var(--color-tip);">chosen</span>
							<div class="h-2 rounded-full" style="background: var(--color-surface-hover);">
								<div
									class="h-full rounded-full"
									style="width: {barPct(row.c, row.r).toFixed(
										0
									)}%; background: var(--color-tip); transition: width 400ms ease;"
								></div>
							</div>
							<span class="text-right text-[11px] font-semibold" style="color: var(--color-text);"
								>{row.fmt(row.c)}</span
							>
							<span class="text-[10px] font-semibold" style="color: var(--color-text-muted);"
								>rejected</span
							>
							<div class="h-2 rounded-full" style="background: var(--color-surface-hover);">
								<div
									class="h-full rounded-full"
									style="width: {barPct(row.r, row.c).toFixed(
										0
									)}%; background: var(--color-text-muted); transition: width 400ms ease;"
								></div>
							</div>
							<span class="text-right text-[11px] font-semibold" style="color: var(--color-text);"
								>{row.fmt(row.r)}</span
...
<style>
	...
	.vote-card.pulse {
		animation: chosen-pulse 550ms ease;
	}
	@keyframes chosen-pulse {
		0% {
			box-shadow: 0 0 0 0 color-mix(in srgb, var(--color-tip) 45%, transparent);
		}
		100% {
