...
<section id="part-5" class="py-10">
	<div class="mx-auto max-w-4xl px-6">
		...
		>
			"One loop, two worlds: the same falling curve teaches one bird to speak and the other to see
			a board."
		</blockquote>
		...
		<div id="section-5-1" class="mb-14">
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				architecture: the same 4-layer, 128-wide, 4-head transformer from Part 3, weight for
				weight in structure. The objective: cross-entropy on the next token, straight from
				Part 4. The optimizer: AdamW, warmup, clipping — same constants. The loop: the exact five
				lines from section 4.2. Nothing in the machine knows whether it's learning bedtime stories
				or chess.
			</p>

			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				The <em>only</em> difference is the stream of token IDs flowing in. Quill eats TinyStories
				through its 512-token byte-BPE vocab; Rook eats random-legal chess games through its
				1931-token UCI move vocab. You'll even see the difference before training starts: Quill's loss begins at
				ln(512) = 6.24 and Rook's at ln(1931) = 7.57 — two different starting heights for no
				deeper reason than vocabulary size, exactly as Part 4 predicted.
			</p>

			<Callout type="important" title="The thesis, one more time">
				"It's all next-token prediction over token streams." Not a slogan — a falsifiable claim,
				and this chapter is the experiment. If one architecture with one objective produces a
				storyteller from story tokens and a chess player from move tokens, the intelligence isn't
				in the wiring. It's in what the data demands the model become in order to predict it.
			</Callout>

			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				Each dashboard pairs the loss curve with the thing loss can't show you: what the model
				actually produces. For Quill, sampled text. For Rook, sampled move sequences and how many
				of those moves are legal. Keep your eye on both halves — the curve tells you learning is
		...
		<div id="section-5-2" class="mb-14">
			...
			<Callout type="note">
				<strong>The Problem:</strong> In a minute you'll press "sample" and get a story. But a
				trained model doesn't emit text — it emits, at every step, 512 probabilities. Turning that
				distribution into one concrete token is a genuine decision, made once per token, and the
				policy you choose changes the output's whole character.
			</Callout>

			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				The obvious policy — always take the most probable token
				(<strong style="color: var(--color-text);">greedy decoding</strong>) — fails in a
				characteristic way: it's deterministic and it loops. Once a small model finds a comfortable
				phrase, the most likely continuation of "the little girl and the little girl" is more
				little girl. The fix is to actually <em>sample</em>: draw the next token at random, in
				proportion to its probability. Now likely tokens come up often, unlikely ones occasionally,
				and the output has texture.
			</p>
...
T = 2.0   →   0.51  0.31  0.19    flattened — underdogs get real chances`}
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				<strong style="color: var(--color-text);">Top-k sampling</strong> keeps only the k most
				probable tokens (say, 40), renormalizes, and samples from those. The tail's collective
				weight gets redistributed to plausible candidates. Its cousin top-p, plus tricks like
				repetition penalties, wait for Part 13 — temperature and top-k are all you need to drive
				the labs.
			</p>
			...
			<Callout type="tip" title="Why teach this before the first sample?">
				...
				should know that's the <em>distribution's</em> fault, not the sampler's. And because when
				you crank the temperature later and watch a trained Quill dissolve back into byte soup,
				you'll recognize what you're seeing: yourself, manually undoing the sharpening that
				training bought.
			</Callout>
		</div>
...
to the pigs. She scared it and did lack. It was hip.`}
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				Now Rook — same architecture, same loop, and the corpus is the only thing that changes.
				Its loss starts higher (ln of 1931) and its progress reads differently: samples go from
				random move tokens toward <em>chess-shaped</em> sequences — openings that look like
				openings, moves that a board would mostly accept. Watch the legal-move gauge rather than
				the raw text; legality is Rook's version of spelling.
			</p>
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				words, then to short grammatical clauses, then to sentences that almost track a thought.
				The gibberish→words→grammar arc takes minutes of accumulated steps, not seconds; this is
				the coffee-break run the course is named for. Sample between rounds so you can feel the
				model move.
			</p>

			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				Three experiments worth running right now: sample at temperature 0 twice (identical
				output, and probably a loop — section 5.2 said so). Crank the temperature high on your
				best-trained Quill and watch it dissolve back toward byte soup. And peek at the per-token
				coloring in the sample stream: the token-level loss heatmap from Part 2's inspector, now
				showing you <em>which</em> predictions are still expensive — surprise retreating from easy
				tokens first.
			</p>
		...
		<div id="section-5-4" class="mb-14">
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				tab. The course's <strong style="color: var(--color-text);">flagship</strong> Quill and
				Rook are 15–25M-parameter versions of the same architecture, trained offline for days on
				the full corpora. You don't get to train them live. You get something arguably better:
				their entire <em>history</em>, saved as checkpoints you can scrub through like video.
			</p>

			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				The idea this teaches: a training run isn't a before and an after — it's a trajectory, and
				abilities arrive along it in a fairly stable order. Early checkpoints of flagship Quill
				make your live bird's mistakes (character statistics, word-shaped noise); mid-run
				checkpoints write clean sentences that forget the character's name; late ones hold a plot.
				Rook's trajectory runs opening moves → mostly-legal middlegames → moves that win material.
				Same slider, same left-to-right arrow of competence.
			</p>
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				One landmark on that trajectory deserves its own name. Fairly early in training, loss
				curves for transformers show a sudden extra drop — a visible bend — that interpretability
				work has traced to the formation of
				<strong style="color: var(--color-text);">induction heads</strong>: pairs of attention
				heads implementing a copy rule — <em>find the last time the current token appeared, look
				at what followed it, and predict that again</em>. It's the moment in-context copying
				clicks. Before it, flagship Quill treats a character's name as a rare token and fumbles it;
				after it, a name introduced once is reused correctly — the model has learned to consult its
				own context instead of relying on global statistics. This bend is marked on the flagship's
				logged training curve; catching it live in your own runs is a bonus, not a promise.
			</p>
		...
		<div id="section-5-5" class="mb-14">
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				same recipe. Every run lands at a different final loss, and when you plot final loss
				against parameter count with both axes logarithmic, the three points sit close to a
				straight line. Three points prove nothing alone; what they demonstrate is the shape of one
				of the most consequential empirical facts in the field:
				<strong style="color: var(--color-text);">loss falls predictably with scale</strong> —
				smoothly, as a power law, across many orders of magnitude of parameters, data, and compute.
			</p>
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				pays for the run. That predictability is why scaling became a strategy rather than a
				gamble — the "bitter lesson" in action: general methods plus more compute keep beating
				clever handcrafted structure.
			</p>
...
tokens per parameter for a compute-optimal run).`}
			...
			<Callout type="warning" title="What scaling laws don't promise">
				They predict <em>loss</em>, not abilities. Section 5.4 showed abilities arriving as jumps;
				a smooth loss forecast tells you nothing about which jumps happen when. Predictable curve,
		...
		<div id="section-5-6" class="mb-8">
			...
			<Callout type="note">
				...
				training signal; an <strong>eval</strong> is a measurement of the thing you actually
				wanted. The gap between them is where all the trouble lives.
			</Callout>
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				<strong style="color: var(--color-text);">legal-move rate</strong> — the gauge you watched
				in 5.3, no judgment calls involved. Strength is measurable: play flagship Rook against
				Stockfish at fixed skill levels and estimate an Elo from the results. Numbers with external
				meaning, cheap to compute, impossible to argue with.
			</p>
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				fix is an <strong style="color: var(--color-text);">out-of-distribution position set</strong>:
			...
			<p class="mb-3 text-[14px]" style="color: var(--color-text-secondary);">
				...
				rubric — grammar, creativity, consistency with the beginning — scored like a teacher
				marking homework. Judged evals scale to thousands of stories and correlate well with human
				ratings, at a price: the judge has taste, and taste has bugs. Judges favor longer answers,
				favor confident phrasing, and can be gamed — a fact that returns with teeth in Part 10,
				because a judge is a reward model wearing a different hat. Later in the course, the judge
				grading <em>your</em> Quill is the course's tutor model, rubric on screen.
			</p>
			...
			<Callout type="tip" title="The eval mindset">
				...
				sets neither bird trains on. Optimize a number long enough and the number detaches from
				the goal — you'll watch that failure happen on purpose in Part 10.
			</Callout>
