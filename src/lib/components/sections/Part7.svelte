	...
	<div class="mx-auto max-w-4xl px-6">
		...
		<div id="section-7-1" class="mb-14">
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				asked it to be useful. Type an instruction at base Quill and it does the only thing it knows
				how to do — treat your instruction as the opening of a story and keep going:
			</p>
...
base Quill:  and a sad ending too. Lily liked to make up stories.
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				It didn't refuse. It didn't fail. It <em>continued</em> — your instruction was just more
				text, and text like that is usually followed by more text like that. The requested words
				never appear, the ending is whatever falls out, and it never stops. This isn't a small
				model being dumb; a raw pretrained frontier model does exactly the same thing, just more
				eloquently.
			</p>
			...
			<Callout type="note" title="The Problem">
				...
				<em>looks</em> like it's obeying, because almost any prompt has plausible continuations that
				resemble compliance. The gap shows up the moment you add a constraint it must actually
				track.
			</Callout>
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				next-token prediction — but on a corpus of (instruction → response) pairs. Nothing about
				the machinery from Part 4 changes. Only the data changes, and the data is the message: text
				where instructions are <em>followed by their answers</em> makes a model that follows
				instructions.
			</p>
...
Story: Once upon a time, there was a brave little frog named Fin.
  ...
  woods...`}
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				thousand of these and Quill learns the conditional distribution: <em>given</em> this recipe,
				produce a story satisfying it. In this course the live birds get a full fine-tune — every
				parameter updated, coffee-break sized — while the flagships get the LoRA treatment from
				section 7.3.
			</p>
		...
		<div id="section-7-2" class="mb-14">
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				stream, formatted by a <strong style="color: var(--color-text);">chat template</strong>.
				The template is not documentation, not a UI convention — it is literal tokens in the
				sequence, and the model was trained on millions of sequences shaped exactly like it.
				Quill's is deliberately tiny:
			</p>
...
<|eos|>`}
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				"&lt;|user|&gt;" out as plain characters. The fence tokens are the model's only ground
				truth about who is speaking.
			</p>
			...
			<Callout type="warning" title="The single most-missed detail: mask the prompt loss">
				...
				spends a large fraction of its gradient learning to <em>generate instructions</em> — it
				gets great at writing "Words: lantern, hop" and mediocre at writing stories. Every
				practical SFT stack masks the prompt:
				<strong>loss is computed only on the response tokens</strong> (and the EOS). The prompt
				tokens still flow through the forward pass — the model conditions on them — they just
...
mask = 1  →  this is the behavior we are paying gradient for`}
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				Notice what's inside the mask: the <Code code="<|eos|>" /> token. That one masked-in token
				is how the model learns to <em>stop</em>. Base Quill rambles until you cut it off, because
				nothing in a pretraining stream ever says "done". After SFT, the model has been trained to
				emit <Code code="<|eos|>" /> when the story completes, and the sampler halts when it sees
				it. Stopping is learned behavior, and it's learned from exactly one token per example.
			</p>
			...
			<Callout type="tip" title="Template mismatch is a real production bug">
				The template used at inference must match the one used in training, byte for byte. A
				missing newline after <Code code="<|assistant|>" /> puts every inference query slightly
				off the training distribution — the model still answers, just measurably worse, and
				nothing errors. Whole GitHub issues full of "fine-tune seems dumb" trace back to this.
				When something feels off, diff the token streams, not the strings.
			</Callout>
		...
		<div id="section-7-3" class="mb-14">
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				an empirical observation: the <em>change</em> a fine-tune makes to a weight matrix is
				approximately low-rank. So don't learn the change as a full matrix; parametrize it as a
				product of two thin ones and learn those:
			</p>
...
α is a scale knob; α/r keeps behavior stable when you change r.`}
  ...
  S --> H(["output h"])`}
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				Concretely: a <Code code="512×512" /> attention projection holds 262,144 weights. A rank-8
				adapter beside it holds <Code code="512·8 + 8·512 = 8,192" /> — about 3% of that layer, and
				since adapters typically attach only to the attention projections, the trainable total
				lands around <strong style="color: var(--color-text);">0.1–1% of the model</strong>.
				Optimizer state shrinks by the same factor. That's what makes flagship fine-tuning fit in
				a tab.
			</p>

			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				The zero-initialization of <Code code="B" /> is the elegant part. At step 0 the bypass
				contributes <Code code="B(A·x) = 0" />, so the adapted model <em>is</em> the pretrained
				model, exactly — training starts from known-good behavior instead of a random perturbation
				of it. But zero-init has a sneaky consequence that bites real engineers:
			</p>
...
After one optimizer step B is nonzero, and A wakes up at step 1.`}
			...
			<Callout type="warning" title="Field report: the zero-gradient scare">
				...
				adapter gradients — for a bad hour it looked like a broken WebGPU autodiff. It wasn't.
				One cause was precisely the math above (zero-init output projections make upstream grads
				mathematically zero at step 0); the others were a frozen zero-init base and a degenerate
				test input that zeroed the softmax backward. The autodiff was correct the whole time.
				When you see zero gradients, check your <em>initialization story</em> before you blame
				the framework — "exactly zero" is a structural clue, "small and noisy" is a numerical
				one.
			</Callout>

			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				Because the base stays frozen, adapters
				<strong style="color: var(--color-text);">swap like cartridges</strong>. Each
				post-training stage of the flagships ships as a small adapter file — under a megabyte —
				against one shared base: slot in the instruct adapter, the preference adapter from Part 9,
				or nothing at all. And "nothing at all" is itself load-bearing:
				...
				anchor a KL penalty to — with LoRA, that copy costs zero extra memory. Flip the adapter
				off and you're looking at the reference.
			</p>

			<Callout type="note" title="What LoRA does not save">
				LoRA shrinks <em>trainable parameters and optimizer state</em> — not compute. The forward
				and backward passes still traverse the full frozen model, and activation memory still
				scales with sequence length and batch size. That's why the in-browser flagship fine-tune
				runs at short sequence lengths with small batches. "Cheap to train" here means cheap in
				memory and state, not cheap in FLOPs.
			</Callout>
		</div>
		...
		<div id="section-7-4" class="mb-14">
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				<strong style="color: var(--color-text);">Elo-bucket token</strong> — the rating band of
				the players. Pretraining on labeled games taught Rook a family of conditional
				distributions: how 1200s play, how 2200s play, selected by one token:
			</p>
...
part of the learned distribution generates the game.`}
			...
			>
				<li>
					<strong style="color: var(--color-text);">Conditioning selects.</strong> The 2200-token
					points at behavior the corpus already contains. Flip the token back to 1200 and weak
					play returns intact — nothing was lost, because nothing was changed. But conditioning
					has a hard ceiling: it cannot summon play stronger than the strongest games the corpus
					showed at that label.
				</li>
				<li>
					<strong style="color: var(--color-text);">Fine-tuning moves.</strong> After elite-only
					SFT, expert play is the <em>default</em> — no token needed. But the distribution
					shifted: the model's grip on weak, weird, human chess fades. You gained a specialist
					and quietly paid for it with range.
				</li>
			</ul>

			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				This is the distinction hiding inside every "prompt engineering vs fine-tuning" debate.
				A system prompt — "you are an expert programmer" — is an Elo token with more words: it
				steers the model toward a region of behavior pretraining already covered. SFT moves the
				whole distribution to live in that region. Prompting is reversible, cheap, and bounded by
				what's already in there; fine-tuning is powerful, persistent, and pays a tax you'll
				measure in the next section.
			</p>

			<Callout type="tip" title="Why base models seemed to follow instructions in 2020">
				Before instruct-tuning existed, people prompted raw GPT-3 with a few examples of a task
				and it "just worked". Now you can name what that was: conditioning. The few-shot examples
				were a soft Elo token — they selected task-shaped behavior already present in a corpus
				that contains Q&amp;A pages, exams, and tutorials. SFT didn't create instruction
				following so much as make it the default, no conditioning required.
			</Callout>
		</div>
		...
		<div id="section-7-5" class="mb-8">
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				picks — the <em>same</em> set every time, chosen before you saw the results. For this first
				boundary, three things should jump out:
			</p>
			...
			>
				<li>
					<strong style="color: var(--color-text);">Constraints bind now.</strong> Ask for
					"lantern" and "hop" and both words appear. Ask for a moral and one shows up at the end,
					in TinyStories' earnest house style.
				</li>
				<li>
					<strong style="color: var(--color-text);">It stops.</strong> Instruct Quill ends its
					story and emits <Code code="<|eos|>" />. Base Quill runs until the token budget cuts
					it off mid-sentence. One masked-in token, visible behavior change.
				</li>
				<li>
					<strong style="color: var(--color-text);">It answers instead of continues.</strong> The
					instruction is treated as a request, not as the opening line of a story about someone
					named Write.
				</li>
			</ul>
			...
			<p class="mb-4 text-[14px] leading-relaxed" style="color: var(--color-text-secondary);">
				...
				<em>pretraining</em> mix — plain stories, no instruction scaffolding — and its loss is
				higher than base Quill's. Not catastrophically; measurably. The distribution moved toward
				instruction-shaped text, and raw story-continuation, the thing pretraining perfected,
				drifted slightly out of focus. Sampled stories also get a touch more uniform: the
				fine-tune's house style smooths over some of the base model's variety.
			</p>

			<Callout type="note" title="The alignment tax, in miniature">
				This trade has a name at frontier scale: the alignment tax. Every post-training stage
				buys a behavior — instruction following now, preference alignment in Part 9 — and pays
				for it somewhere in the base distribution. Labs measure the tax on benchmark suites; you
				just measured it on a held-out loss. The pattern repeats at every stage boundary from
				here on, which is exactly why the before/after ritual exists: never let a stage tell you
				only what it improved.
			</Callout>
